...
- **SWOB look-back must stay ≥ the reporting interval of the stations you care about.** Marine sites (KELP REEFS, DISCOVERY ISLAND, Victoria Harbour) report *hourly*; a 20-min window silently caught them in ~1 run of 4 and made stations flicker in and out. `WINDOW_MIN` now tracks `STALE_MIN` (60). Don't shrink it to "save bandwidth" — the whole response is ~27 KB.
