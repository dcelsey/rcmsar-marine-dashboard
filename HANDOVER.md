...
- **Cron**: `.github/workflows/fetch-wind.yml` runs on `workflow_dispatch` from a Cloudflare Worker (`fetch-wind-trigger`) every 15 min, plus an hourly `23 * * * *` GH schedule as a fallback. Node 22, checkout/setup-node v5; pushes `chore(wind): refresh live observations [skip ci]` commits by `wind-bot` when the JSON changes. The Worker source lives in [cf-workers/fetch-wind-trigger/](cf-workers/fetch-wind-trigger/); it POSTs to the workflow_dispatch endpoint using a fine-grained PAT stored as the `GH_TOKEN` secret in CF. **PAT rotation** — fine-grained PATs cap at 1 year, so plan to rotate around **2027-07**; if CF logs start showing 401s from GH, that's the trigger.
...
- **Vercel**: **connected and live at `https://rcmsar-marine-dashboard.vercel.app`** (this entry previously said "not yet connected" — that was wrong; it has been deploying every 15 min for weeks). Two things about it are counter-intuitive and cost a session to work out, both logged as **CR-005**: (1) plain code pushes to `main` do **not** trigger a deploy — code reaches production only by riding the next wind-bot commit, so expect up to a 15-min lag after pushing; (2) Vercel here ignores **both** `[skip ci]` and `[skip vercel]`, so commit-message tokens can't be used to cut the ~96 deploys/day (vs the Hobby tier's 100/day cap).
...
- **ECCC GeoMet (`api.weather.gc.ca`) is intermittently flaky.** It returns both `fetch failed` and spurious `HTTP 404`s on URLs that succeed seconds later — reproduced twice on 2026-07-30 while fixing CR-002. Assume any single call can fail for no reason. `scripts/fetch-wind.mjs` handles this by carrying each source forward from the previous `wind.json` when its fetch fails (bounded by `STALE_MIN`), and sets `degraded: true` on the payload so the failure is visible in the committed JSON rather than only in Actions logs. **If you add another GeoMet-backed feed, give it the same treatment** — a partial write that looks structurally valid is far more dangerous than an outright failure, because nothing alerts.
- **Three separate knobs govern whether a wind station appears — don't conflate them** (all in `scripts/fetch-wind.mjs`; the full story is CR-004):
  - `WINDOW_MIN` (180) decides what we *can* see. It must be wide enough to hold **two** obs from an hourly station, because that's how `medianCadenceMin` infers cadence. At 120 it intermittently held one, cadence came back `null`, and the hourly marine sites vanished.
  - `stale` decides what actually *renders* — and it's **cadence-aware**: `age > max(STALE_MIN, cadence + 15)`. A station is stale when it has missed a report, not merely when its reading is old. A flat 60-min bound is incompatible with a station that reports every 60 min: KELP REEFS/DISCOVERY ISLAND/Victoria Harbour are never fresher than ~60 min, so they were hidden ~6 min of every hour. A 1-min station that dies is still stale at 60 min, so the SAR-critical bound is intact where it means something.
  - `SWOB_PROPS` keeps the wide window affordable — 25 MB → 1.8 MB. **One unknown property name 400s the whole query**, so check any field you add against the collection schema.
- **`stale` is baked in at write time**, so a bad snapshot persists for the whole deploy cycle (~15 min), not until the next fetch. That's what turned a 6-minute upstream gap into a 15-minute outage on the dashboard.
- **Some ECCC sites publish under two `msc_id`s at identical coordinates** (Victoria Harbour, Tofino, Nanaimo, Abbotsford, Campbell River, Powell River, Masset, Smithers, Bella Coola — an AUTO feed plus its staffed/AWOS partner). Keyed by `msc_id` alone they both survive and stack two markers on the same pixel. `dedupeByPosition` in `fetch-wind.mjs` collapses them; keep that in mind for any other per-station map layer.
...
- **GH Actions scheduled runs are unreliable** — we tried `*/15`, then offset `:07/:22/:37/:52`, then dense `*/5`, and still saw GH drop ~7 of 8 slots during peak-load windows. The fetch-wind trigger is now a Cloudflare Worker calling `workflow_dispatch` (which is reliable); GH `schedule:` is kept only as a hourly fallback. Dispatch-triggered runs show as "manually triggered" in the Actions UI — that's just how GH labels every `workflow_dispatch` regardless of caller.
