...
- **SWOB look-back must be *strictly larger* than `STALE_MIN`, not equal to it.** Marine sites (KELP REEFS, DISCOVERY ISLAND, Victoria Harbour) report *hourly* and ECCC publishes ~5–6 min after the hour. `WINDOW_MIN` currently equals `STALE_MIN` (60), which leaves no slack: a fetch between :00 and :06 sees the previous ob as just outside the window and the current one as not yet published, so those stations vanish entirely — and whichever snapshot deploys freezes for 15 min. **This is a live, known bug — see CR-004**, which has the diagnosis, the measured cost of a 120-min window, and an untested `&properties=` idea for keeping the payload small. The rule: the fetch window decides what you *can* see, `stale` decides what you *should* see. Don't conflate them.
